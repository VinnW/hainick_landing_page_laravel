...
const ActivityPanel = () => {
    ...
    const handleDropOnSlot = (e, slot) => {
        ...
        if (
            Number(draggedRow.is_active) === 1 &&
            draggedRow.image_type === slot
        )
            return;
                ...
                if (willDisplace && r.id === occupant.id) {
                    if (draggedWasActive && draggedOldImageType) {
                        return {
                            ...r,
                            is_active: 1,
                            image_type: draggedOldImageType,
                        };
                    }
        ...
        try {
            ...
            for (const pr of changed) {
                ...
                if (!res.ok) {
                    ...
                    throw new Error(
                        errData.error || `Gagal update ID ${pr.id}`,
                    );
                }
            }
    ...
    const handleDelete = async (row) => {
        ...
        try {
            const res = await fetch(
                `${API_URL}/delete-updates-section/${row.id}`,
                {
                    method: "DELETE",
                },
            );
      ...
      `}</style>
            ...
            <div className="ap-wrap">
                ...
                <div className="ap-header">
                    <div className="ap-header-left">
                        ...
                        <p className="ap-page-sub">
                            Drag kartu ke slot posisi untuk mengaktifkan · maks.
                            5 aktif
                        </p>
                    </div>
                    <button
                ...
                <div className="ap-legend">
                    ...
                    <span>
                        <strong>Cara pakai:</strong> Drag kartu dari{" "}
                        <em>Non-aktif</em> ke slot posisi yang diinginkan di
                        zona <em>Aktif</em> untuk menampilkannya di landing
                        page. Drag kartu aktif ke zona <em>Non-aktif</em> untuk
                        menyembunyikannya. Drag kartu aktif ke slot lain untuk
                        menukar posisinya. Setelah selesai mengatur, klik{" "}
                        <strong>Simpan Perubahan</strong> untuk menerapkan ke
                        tampilan depan.
                    </span>
                </div>
                        ...
                        <div>
                            ...
                            <div className="ap-active-zone">
                                <div className="ap-slots-top">
                                    {IMAGE_TYPE_OPTIONS.slice(0, 3).map(
                                        (slot) => (
                                            <ActiveSlot
                                                key={slot}
                                                slot={slot}
                            ...
                            >
                                {inactiveRows.length === 0 ? (
                                    <div className="ap-inactive-empty">
                                        ...
                                        <br />
                                        Drag kartu aktif ke sini untuk
                                        menyembunyikannya.
                                    </div>
                                ) : (
        ...
        >
            {!card ? (
                ...
                <div
                    ...
                    onDragEnd={onDragEnd}
                >
                    <img src={`${BASE_URL}${card.image_url}`} alt={label} />
                    <span className="ap-slot-badge">{label}</span>
                    {isPending && (
                        <span
                            className="ap-slot-pending-dot"
                            title="Belum disimpan"
                        />
                    )}
                    <div className="ap-slot-card-overlay">
                        <span className="ap-slot-card-drag-hint">
                            ⠿ Drag untuk pindah
                        </span>
                        <div className="ap-slot-card-actions">
                            <button
                                className="ap-slot-card-btn"
                                onClick={() => onEdit(card)}
                            >
                                Edit
                            </button>
...
function InactiveCard({
    ...
    return (
        ...
        >
            {row.image_url ? (
                <img src={`${BASE_URL}${row.image_url}`} alt="" />
            ) : (
                <div
                    style={{
                        width: "100%",
            ...
            <div className="ap-inactive-card-overlay">
                <span className="ap-inactive-card-drag-hint">
                    ⠿ Drag ke slot aktif
                </span>
                <div className="ap-inactive-card-actions">
                    ...
                    <button
                        className="ap-inactive-card-btn del"
                        onClick={onDelete}
                    >
                        Hapus
                    </button>
...
const ActivityModal = ({
    ...
    const [preview, setPreview] = useState(
        data?.image_url ? `${BASE_URL}${data.image_url}` : null,
    );
    const [saving, setSaving] = useState(false);
    ...
    const handleSave = async () => {
        ...
        try {
            if (isEdit) {
                // Update gambar menggunakan endpoint by ID agar tidak menimpa row lain
                ...
                if (imageFile) {
                    ...
                    const res = await fetch(
                        ...
                        {
                            method: "PUT", // ❌ PUT asli + FormData → Laravel gak bisa parse
                            body: fd,
                        },
                    );
                    if (!res.ok) {
                        const fd2 = new FormData();
                        fd2.append("image_url", imageFile);
                ...
                if (description !== (data?.description || "")) {
                    await fetch(
                        `${API_URL}/update-updates-section-description`,
                        {
                            method: "PUT",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({
                ...
                // New record selalu is_active=0, biarkan user drag ke slot
            ...
            <div className="ap-modal">
                ...
                <div className="ap-modal-body">
                    ...
                    {!isEdit && activeCount >= 5 && (
                        ...
                        >
                            ⚠️ Sudah ada 5 activity aktif. Activity baru akan
                            disimpan sebagai <strong>non-aktif</strong>.
                        </div>
                    )}
                </div>
